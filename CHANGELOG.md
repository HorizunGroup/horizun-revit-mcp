...
- **Live matrix, 2026-09-26, at b9aee80** (run-year-matrix + verify-live with -WriteProbes, add-in built per year from a clean tree): **2023, 2024, 2026 and 2027 green, 374 passed / 0 failed each**, including the Navisworks handoff (5/5), spatial links, change_type_by_rule, the closed-workset fixture, the S/M/L workloads and case 15 with a staged link in 2023. The first sweep at 11caa65 caught two real defects the review batch had introduced or hidden - joined walls left unmeasured by an "exactly four edges" rule, and a probe that applied a token-less import through the token path - both fixed (6fa6278, b9aee80) and re-measured. Revit 2025 was not run: a user session was open in it, and the driver does not touch a session it did not start. The "not covered" rows are fixture limits (no workshared write model, no panel, no design options).

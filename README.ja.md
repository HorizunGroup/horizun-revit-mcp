...
- **デフォルトでリバーシブル。** 変更を伴うワークフローは `revit_batch_execute` を通じて実行でき、複数のコマンドを 1 つの Revit `TransactionGroup` にラップすることで、1 回の元に戻す操作でバッチ全体をロールバックできます。
...
2. 高度な自動化が必要な場合は、デフォルトのツールサーフェスから直接 `revit_send_code_to_revit` を呼び出します。
3. アダプティブベイクが有効な場合、繰り返し行われるローカル使用パターンが `%LOCALAPPDATA%\RvtMcp\` の下にローカル記録されます。
4. 繰り返しパターンが提案として表示され、`revit_list_bake_suggestions` で確認できます。
5. ツール名、スキーマ、出力先を指定して、`revit_accept_bake_suggestion` で明示的に提案を承認します。
6. 承認されたツールは `revit_list_baked_tools` / `revit_run_baked_tool` で呼び出し可能になり、Revit リボンランタイムキャッシュからも利用できます。
...
│   ├── plugin-r22/              # Revit 2022 シェル — .NET 4.8、TCP
│   ├── plugin-r23/              # Revit 2023 シェル — .NET 4.8、TCP
│   ├── plugin-r24/              # Revit 2024 シェル — .NET 4.8、TCP
│   ├── plugin-r25/              # Revit 2025 シェル — .NET 8、Named Pipe
│   ├── plugin-r26/              # Revit 2026 シェル — .NET 8、Named Pipe
│   └── plugin-r27/              # Revit 2027 シェル — .NET 10、Named Pipe
...
4. `revit_get_current_view_info` を呼び出します。
...
全サーフェスは **23 ツールセットにわたる 223 ツール**（`--toolsets all`）です。デフォルトでは `modify` と `delete` を除くすべてのツールセットが有効です。アダプティブベイクが有効な場合、承認されたベイクドツールが追加で読み込まれます。`--read-only` は書き込み可能なすべてのツールセットを削除します。
...
| `meta` | `revit_show_message`、`revit_switch_target`、`revit_batch_execute`、使用状況統計、プロジェクト情報設定、未使用ファミリ削除 | オン |
...
| `modify` | `revit_operate_element`、`revit_color_elements`、パラメータ/タイプ/ワークセット編集 | オフ |
| `delete` | `revit_delete_element` | オフ |
| `annotation` | 要素/カテゴリタグ付け、テキスト注釈、寸法、塗り潰し領域、詳細線、コールアウト、キーノート、未タグ/未寸法チェック、空タグ削除 | オン |
| `export` | `revit_export_room_data` | オン |
| `mep` | `revit_detect_system_elements` | オン |
...
以下の表は代表的なツールに焦点を当てています。全サーフェスは 23 ツールセットにわたる 223 ツールです。

| ツールセット | ツール | 説明 |
|---|---|---|
| `query` | `revit_get_current_view_info` | アクティブビューのメタデータ：タイプ、レベル、縮尺、詳細レベル。 |
| `query` | `revit_get_selected_elements` | ID、名前、カテゴリ、タイプを含む現在選択中の要素。 |
| `query` | `revit_get_available_family_types` | プロジェクト内のファミリタイプ、カテゴリでフィルタ可能。 |
| `query` | `revit_ai_element_filter` | カテゴリとパラメータ/演算子でフィルタ、値は mm 単位。 |
| `query` | `revit_analyze_model_statistics` | カテゴリ別にグループ化された要素数。 |
| `query` | `revit_get_material_quantities` | カテゴリの面積と体積の合計。 |
| `query` | `revit_get_element_details` | 要素メタデータ、位置、バウンディングボックス、ワークセット、フェーズ、グループ、アセンブリ ID。 |
| `query` | `revit_get_element_parameters` | ストレージタイプ、表示値、生の値、データ/スペック ID を含むインスタンスパラメータ。 |
| `query` | `revit_get_type_parameters` | タイプ ID または要素 ID からのタイプパラメータ。 |
| `query` | `revit_list_project_parameters` | プロジェクト/共有パラメータバインディング、バインディング種別、カテゴリ。 |
| `query` | `revit_get_element_relationships` | ホスト、グループ、アセンブリ、所有ビュー、設計オプション、ネスティング、従属要素。 |
| `query` | `revit_list_groups` | タイプ、添付/詳細メタデータ、オプションのメンバー ID を含むグループインスタンス。 |
| `query` | `revit_get_group_members` | カテゴリ、タイプ、所有ビュー、固定状態を含むグループインスタンスのメンバー。 |
| `query` | `revit_list_assemblies` | タイプ、命名カテゴリ、メンバー数、オプションのメンバー ID を含むアセンブリインスタンス。 |
| `query` | `revit_get_assembly_members` | カテゴリ、タイプ、グループ、ワークセット ID を含むアセンブリインスタンスのメンバー。 |
| `query` | `revit_list_worksets` | ワークセット、アクティブワークセット、編集/オープン状態、オプションの要素数。 |
| `create` | `revit_create_line_based_element` | 壁またはその他のライン基準要素。 |
| `create` | `revit_create_point_based_element` | ドア、窓、家具、またはその他のポイント要素。 |
| `create` | `revit_create_surface_based_element` | ポリラインから床または天井を作成。 |
| `create` | `revit_create_level` | 標高（mm）でのレベルの作成。 |
| `create` | `revit_create_grid` | 2 点間（mm）のグリッドライン。 |
| `create` | `revit_create_room` | 壁で囲まれたポイントにルームを作成。 |
| `create` | `revit_create_group_from_elements` | 2 つ以上の要素からモデル/詳細グループを作成。 |
| `modify` | `revit_operate_element` | ID に対して選択、非表示、表示、分離、色設定を実行。 |
| `modify` | `revit_color_elements` | パラメータ値に基づいてカテゴリを色分け。 |
| `modify` | `revit_set_element_parameter_values` | 複数の要素にまたがってインスタンスパラメータを設定。 |
| `modify` | `revit_set_type_parameter_values` | 明示的または要素解決済みタイプにまたがってタイプパラメータを設定。 |
| `modify` | `revit_change_element_type` | 要素を互換性のあるターゲットタイプに変更。 |
| `modify` | `revit_assign_elements_to_workset` | 共有モデル内のユーザーワークセットに要素を割り当て。 |
| `delete` | `revit_delete_element` | ID リストで削除。明示的に必要な場合を除きオフにしておくこと。 |
| `view` | `revit_create_view` | フロアプランまたは 3D ビュー。 |
| `view` | `revit_place_view_on_sheet` | ビューを新規または既存のシートに配置。 |
| `view` | `revit_analyze_sheet_layout` | タイトルブロック、ビューポート位置、縮尺（mm）。 |
| `export` | `revit_export_room_data` | 名前、番号、面積、周長、レベル、体積を含むルームデータ。 |
| `annotation` | `revit_tag_all_walls` | 中央点に壁タイプタグを配置。既存タグはスキップ。 |
| `annotation` | `revit_tag_all_rooms` | 位置点にルームタグを配置。既存タグはスキップ。 |
| `mep` | `revit_detect_system_elements` | シードからコネクタを辿り、システムメンバーを返す。 |
| `toolbaker` | `revit_send_code_to_revit` | デフォルトのツールサーフェスから、Revit 内でアドホック C# をコンパイル・実行。 |
| `toolbaker` | `revit_list_baked_tools` | 承認済みパーソナルベイクドツールの一覧。 |
| `toolbaker` | `revit_run_baked_tool` | 承認済みベイクドツールを名前で呼び出し。 |
| `toolbaker` | `revit_list_bake_suggestions` | アダプティブベイクのみ：ローカル提案の一覧。 |
| `toolbaker` | `revit_accept_bake_suggestion` | アダプティブベイクのみ：ローカル提案を承認・適用。 |
| `toolbaker` | `revit_dismiss_bake_suggestion` | アダプティブベイクのみ：ローカル提案をスヌーズまたは却下。 |
| `meta` | `revit_show_message` | Revit 内の TaskDialog — 接続テストや通知用。 |
| `meta` | `revit_switch_target` | 複数バージョン実行時にアクティブな Revit 接続を切替。 |
| `meta` | `revit_batch_execute` | 1 つの `TransactionGroup` 内でコマンドを原子的に実行。 |
| `meta` | `revit_analyze_usage_patterns` | ローカル使用統計：ツール呼び出し、セッション、エラー。 |
| `lint` | `revit_analyze_view_naming_patterns` | 主要なビュー命名パターンと外れ値を推測。 |
| `lint` | `revit_suggest_view_name_corrections` | ビュー外れ値に対する修正名を提案。 |
| `lint` | `revit_detect_firm_profile` | プロジェクト命名をファームプロファイルと照合。 |
...
- **ToolBaker 制御。** `revit_send_code_to_revit` はデフォルトで利用可能です。アダプティブベイクはオプトイン制であり、提案/ログ機能のみを制御します。`--read-only` または `--disable-toolbaker` により ToolBaker サーフェス全体が削除されます。
...
JSON ファイルのパス：`%LOCALAPPDATA%\RvtMcp\rvtmcp.config.json`。

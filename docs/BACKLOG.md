...
### From a full day of real homologation use — 2026-08-05 (9 customer-library families, add-in 0.5.0→0.6.1)

Measured in the field, not predicted. Ordered by what cost time or nearly caused a wrong claim.

| ID | Story | Size | Dep |
|----|-------|------|-----|
| 5.12 ✅ | **DONE + LIVE-VERIFIED 2026-08-08 on Revit 2024** (PR #58, merged `c020268`). `FileFacts` moved to Core with `save_document`'s share mode (`FileShare.ReadWrite\|Delete`) — the bug was one line, `File.OpenRead()`, which demands the exclusive access Revit never grants on the document this evidence is taken of. Live: `family_apply save:true` on a family Revit held open answered `file_changed: true` with BOTH sha256 hashes non-null — and size_before == size_after on that save, so the content hash was the only field that COULD prove it. Was: `file_changed: null` 9/9, "file is being used by another process" | S | — |
| 5.13 ✅ | **DONE + LIVE-VERIFIED 2026-08-07** (PR #51, merged `15cbcdd`). `close` takes `activate_other: true`: it activates another open document — the first whose path exists on disk — or opens the bridge's own anchor project when nothing else qualifies, then closes the target and REPORTS which it activated. Live on Revit 2025, all three branches: single doc open → `created_anchor` (`HZ_ANCHOR_2025.rvt` minted); anchor already open → `activated_open_document` picking it; A+B+anchor open, close active B → `activated_open_document` picking the real model `HZ25_LIVE_A`, not the anchor. Was: **Closing the last document requires a decoy.** The only workaround was opening a document you do not want, three times in one session. **Re-confirmed 2026-08-07 at batch scale**: the last model of a 54-model batch stays open, so RELAUNCHING the batch skips it | S | — |
| 5.14 ✅ | **DONE + LIVE-VERIFIED 2026-08-08 on Revit 2024** (PR #59, merged `53fbde6`). `would_change` tri-state per row of `params_would_set`, computed once at plan time (`Core/WouldChange.cs`): true would move it, false already holds it (still written — idempotent — the note says so), null cannot be told with the reason beside it (the big null: a unit-aware string parsed inside Revit at apply time — guessing false hides a write, guessing true invents one). Live, both branches: fresh values → `would_change=true` with the before shown; after the apply, the same dry run → `would_change=false` on both rows | S | — |
| 5.15 ✅ | **DONE + LIVE-VERIFIED 2026-08-08 on Revit 2024** (PR #63 merged `e89db88`, + #64 `e611561` from the verification itself). `horizun_acc_upload_status`: reads the Desktop Connector's own log and answers `has_folder_urn` per name — the connector's testimony, not a cloud API check; a miss is absence of EVIDENCE, never proof of absence. Organisation-neutral where the field script was not (names are arguments, no customer-specific prefix compiled in); rule in `Core/AccUploadWal.cs`. The live verification found and fixed its own gap: this machine's newer DC names the log `<urn>.properties.db`, not `*.properties-log.db` — both swept now. Live against the real WAL: 12 logs read 0 unread, a real cloud model matched (`has_folder_urn=true`, 21 folders ALL reported), a never-uploaded name `false`, missing-root and empty-folder refusals exact. Was: 3 of 8 families silently unuploaded, caught only by a human screenshot | M | — |
| 5.16 ✅ | **DONE + LIVE-VERIFIED 2026-08-08 on Revit 2024** (PR #60, merged `814ff64`). The transport records each pipe connection's client pid (Win32 `GetNamedPipeClientProcessId`, presence only — auth stays with token+ACL) into `Core/ClientPresence` (10-min window); health answers `other_clients_connected` = distinct − 1, floored at 0, with per-client pid/name/age/alive rows. An unreadable pid is counted as unidentified and declares the count a lower bound. Live, the measured two-agent case: first client alone → 0; a second client's health call → `other_clients_connected=1`, the first client's pid listed with `process_alive=false` — the one line that would have replaced three journal autopsies | S | — |
| 5.17 ✅ | **DONE + LIVE-VERIFIED 2026-08-08 on Revit 2024** (PR #61, merged `2c592c0`). Every dimension names its unit (`GeoUnits` in Core: ft3/ft2/ft/count; unknown answers "unknown", never a guess) and both `geometry_baseline` and `geometry_check` carry a units note. AND the 656.17 mystery is solved: it is 200 m in feet — the metric template's REFERENCE PLANES, whose bounds the capture was folding into the box. Only elements that contributed a solid extend the bbox now. Live: baseline read `solid_volume 5.65 [ft3]`, `bbox 2.62×2.62×0.82 [ft]` — the piece, not the scaffolding | S | — |
| 5.18 ✅ | **DONE 2026-08-07, vocabulary live on 2024 2026-08-08** (PR #62, merged `699428e`). The honest word is `unproven` — the same one the parameter-schema invariant already uses. `changed` still wins first (a type appearing/vanishing IS an observation); zero comparisons is `unproven`; only then unchanged/unchanged_where_measured — which also fixes the subtler sibling, both sides present with every dimension unreadable. Every consumer was checked before the string changed: the rollback gate reads `AnyChange`/`ActiveTypeFullyMeasured`, never the string. **Live caveat, stated honestly:** a real family always yields dimensions, so the empty-capture case cannot be provoked against a live model — proven by the unit tests; what the live run shows is the shipped vocabulary (`status_means` names all four values in the 2024 reply) and a real `unchanged` with 7 compared beside it | S | — |
...
| ID | Story | Size | Dep |
|----|-------|------|-----|
| 5.19 ✅ | **DONE + LIVE-VERIFIED 2026-08-07** (PR #52, merged `da082b2`). Cuts (a) and (c) shipped: `horizun_health` has a 30 s ceiling, and the dispatcher probes for a pre-existing modal off the UI thread (`ModalProbe` Win32) and returns it as a RESULT once it persists 3 probes (`ModalSighting`). Cut (b), per-call `timeout_ms`, deliberately deferred — the other two cover the measured case. Live on Revit 2025: with a real "New Project" dialog up, `horizun_health` answered in **3107 ms** naming `'New Project' [#32770]`, "NEVER STARTED"; dismissed, it returned **healthy in 104 ms**. The verified trap held: `Interference` cannot see a modal that predates the call (the command never reaches the UI thread), which is exactly why the probe runs transport-side. Was: three `horizun_health` calls each waited the full 600 s behind one dialog — 30 minutes to learn what the log knew in the first second | M | — |
| 5.20 ✅ | **DONE + LIVE-VERIFIED 2026-08-07** (PR #57, merged `b6cb661`). New typed read-only command `horizun_file_info`: `paths[]` or `folder` (swept for `pattern`, default `*.rvt`, optional `recursive`), reading `BasicFileInfo` off disk — format/saved version, `is_workshared`, `is_central`, `is_local`, central path — with NOTHING opened and NO active document. The WHICH-files rule is `FileInfoPaths` (Revit-free, unit-tested: refusal, union, dedup, cap at 2000, throwing lister); the reader is `BasicFileProbe`. Live on Revit 2025 with **zero documents open**: swept `C:\hz-live`, 14/14 readable, each with its true version (2025, 2023, 2026), `is_workshared`/`is_central`. Owner lifted the freeze for this one command. Was: format triage needed no document, yet an anchor project had to be created by hand because the bridge demanded an active one | S | freeze lifted by owner |
| 5.21 ✅ | **DONE + LIVE-VERIFIED 2026-08-07** (PR #53, merged `871e3f2`). `Job.Result` stores `revit_said` on the result event (built exactly as `PipeEnvelope`), `RunOneAsync` serialises it — on failure too, since it is usually the reason — and `job_status` surfaces it as a sibling of `result`; absent reads as null ("Revit raised nothing"), never "dropped". Live on Revit 2025: an async `execute_python run_async` job whose script raised a `TaskDialog` came back through `job_status` carrying `revit_said` with `dialogs:1` and the cancelled dialog named, beside the result. (The batch path is `execute_python run_async`, not `submit_job`, which refuses `execute_python` by name.) Was: **`revit_said` is dropped on the async path.** The dialog/warning telemetry that solved the day's hardest case existed for a synchronous call and vanished for the same work run async — four failed models went undiagnosed for exactly this | S | — |
| 5.22 ✅ | **DONE + LIVE-VERIFIED (partial) 2026-08-07** (PR #56, merged `f3a71f7`). `on_open_dialog: cancel\|dismiss` on `open_document` and `document_session` open. `dismiss` presses OK/continue on an open-time dialog and records it in `revit_said`, via a ThreadStatic policy `Interference` reads at dialog time, scoped to the open call — every other dialog still cancels. Parse rule in `OpenDialogPolicy` (Revit-free, unit-tested); a mistyped value is an error. Live on Revit 2025: a bad value (`acknowledge`) was refused with the exact message, nothing opened; `dismiss` on a clean open of a 2025 model opened fine (no regression). **Residual, stated honestly:** a real open-time dialog could not be forced deterministically in this environment, so "dismiss actually continues past one" is proven by construction (the watcher answers 1 instead of 2) rather than by a live dialog. `IDOK`/`TaskDialogResult.Ok`=1 is best-effort; a dialog whose continue is another button is recorded as answered | M | — |
| 5.23 ✅ | **DONE + LIVE-VERIFIED 2026-08-07** (PR #54, merged `ea4c8ca`). `self_reported_verified` is a declared status, held to the same bar as `verified` (checked=true + non-empty evidence, else downgrade), no longer falling into the unknown-status branch. Live on Revit 2025: an async script declaring `status:"self_reported_verified"` with evidence came back through `job_status` as `evidence_status: self_reported_verified` with the host-not-verified disclaimer — not downgraded, not "is not one of". Was: the classifier warned on the very word every doc and disclaimer popularises — the vocabulary refusing what it taught | S | — |
| 5.24 ✅ | **DONE + LIVE-VERIFIED 2026-08-07** (PR #55, merged `7b41bd7`). The server sweeps at startup via `PipeClient.SweepStaleDiscovery`, deciding WHICH files with `DiscoverySweep` — the Revit-free rule the add-in's `SweepStale` now shares, carrying both refusals (legacy two-segment names never touched, any live pid kept). Live on Revit 2025: a planted orphan `revit-2025-999999.json` (dead pid) was swept by a fresh server at startup while the live `revit-2025-42860.json` was kept | S | — |
...
> **Freeze update (2026-08-07):** the owner lifted it deliberately to clear the
> 5.x field-report stories in one pass. `horizun_file_info` (5.20) landed under
> that decision — a read-only command that opens nothing, which is the lowest-risk
> shape a new tool can have. The freeze's intent (don't pile mutating surface on
> unverified write paths) still holds for Epic 1's commands.

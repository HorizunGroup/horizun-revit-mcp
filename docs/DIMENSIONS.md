...
| references that resolve into an RVT link | typed refusal / structured `link_references_not_supported` | consuming linked references in dimension creation is not proven live; an unproven reference offered as usable is a dimension that fails at apply time |

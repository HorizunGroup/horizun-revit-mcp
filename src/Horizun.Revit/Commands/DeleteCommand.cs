...
//   * IT COMMITS THROUGH Guard.Commit. That single line is what turns the
...
// back empty — instead of the `for _ in range(3)` a script typically does, or the
// "run Manage > Purge Unused by hand 2-3 times" a human falls back to. Each
...
namespace Horizun.Revit.Commands
{
  ...
  ""properties"": {
    ...
    ""id_cap"": { ""type"": ""integer"", ""default"": 200, ""minimum"": 1,
        ...
        /// <summary>
        /// Name the document that was actually touched, and - on a rehearsal - hand back
        /// the key to this exact plan.
        ///
        /// Done here rather than inside Report so the reply carries it whichever path
        /// produced it, and so a future command that reports differently cannot forget.
        /// </summary>
        private static CommandResult Stamp(CommandResult result, GateResult gate, bool dryRun, string planHash)
        {
        ...
        /// <summary>
        /// A fingerprint of what this request asks for. Ids are sorted so the same set in
        /// a different order is the same plan, and every field that changes the scope is
        /// in it - a confirmation must not survive a change to any of them.
        /// </summary>
        private static string PlanHashOf(string mode, JObject request)
        {
            var parts = new List<string> { "mode=" + mode };

            parts.Add("ids=" + JoinSorted(request["ids"] as JArray));
            parts.Add("protect=" + JoinSorted(request["protect_ids"] as JArray));
            parts.Add("categories=" + JoinSorted(request["categories"] as JArray));
            parts.Add("max_passes=" + (request["max_passes"]?.ToString() ?? "-"));

            // id_cap and transaction_name are NOT in the hash. Measured by using this:
            // approving a plan with id_cap=3 and executing it with id_cap=2 was refused as
            // "the plan CHANGED", which is false - id_cap only controls how many rows are
            // LISTED and cannot alter what is deleted. A guard that fires on a display
            // option teaches callers to distrust it, and a guard nobody trusts gets worked
            // around. Only fields that change WHAT IS AFFECTED belong here.
            return ConfirmationStore.HashPlan(parts.ToArray());
        }

        private static string JoinSorted(JArray arr)
        {
            if (arr == null) return "-";
            var items = new List<string>();
            foreach (var t in arr) items.Add(t?.ToString() ?? "");
        ...
        public CommandResult Execute(UIApplication app, string paramsJson)
        {
            ...
            // WHICH model. A delete aimed at "the active document" is a delete aimed at
            // whatever window was in front when the call arrived. target_document is
            // required and must name the document that is actually active.
            ...
            // dry_run defaults to TRUE in BOTH modes now. It used to default to false when
            // ids were supplied, on the reasoning that an explicit list is an explicit act -
            // but an explicit list of ids computed by something else, against a model that
            // has moved since, is exactly the case where a rehearsal is worth its cost.
            // Deleting is not undoable from here.
                ...
                try
                {
                    ...
                    while (passesRun < maxPasses)
                    {
                        ...
                        using (var tx = new Transaction(doc, txName + " (pass " + passesRun + ")"))
                        {
                            ...
                            Guard.Commit(tx, txName + " (pass " + passesRun + ")");
                        }
        ...
        /// so this does NOT go through Guard.
        ...
        private static List<Target> BuildTargets(Document doc, List<long> ids, List<long> protectedIds, JArray rejected)
        {
            ...
            foreach (var raw in ids)
            {
                ...
                if (!Rid.CanRepresentElementId(raw))
                {
                    rejected.Add(new JObject { ["id"] = raw, ["error"] = Rid.ElementIdRangeError(raw) });
                    continue;
                }
        ...
        /// change to the model's warning list, so it goes in the response: a delivery
        ...
        private class SwallowNothingPreprocessor : IFailuresPreprocessor
        {
            ...
            private bool CausedByUs(FailureMessageAccessor f)
            {
                ...
                try
                {
                    ...
                    {
                        long raw;
                        try { raw = Rid.GetId(id); } catch { continue; }

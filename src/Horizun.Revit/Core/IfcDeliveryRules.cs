...
namespace Horizun.Revit.Core
{
    ...
    public static class IfcDeliveryRules
    {
        ...
        /// <summary>
        /// fields joined in field_order by separator. field_order is REQUIRED: a JSON
        /// object's key order is not a naming convention, and guessing it would name the
        /// file in an order nobody agreed to. Each field must be present, non-empty, free of
        /// the separator, and - when field_patterns names it - match that pattern as a WHOLE
        /// value.
        /// </summary>
        public static string ComposeContainerName(JObject container, out string error)
        {
            error = null;
            var fields = container?["fields"] as JObject;
            var order = container?["field_order"] as JArray;
            if (fields == null) { error = "information_container.fields must be an object."; return null; }
            if (order == null || order.Count == 0)
            {
                error = "information_container.field_order must list the fields in name order; key order is not a convention.";
                return null;
            }
            string separator = container.Value<string>("separator") ?? "-";
            if (separator.Length == 0) { error = "information_container.separator must not be empty."; return null; }
            var patterns = container["field_patterns"] as JObject;
            var parts = new List<string>();
            var seen = new HashSet<string>(StringComparer.Ordinal);
            foreach (JToken token in order)
            {
                string field = token.Type == JTokenType.String ? (string)token : null;
                if (string.IsNullOrWhiteSpace(field)) { error = "information_container.field_order holds a non-string entry."; return null; }
                if (!seen.Add(field)) { error = "information_container.field_order names '" + field + "' twice."; return null; }
                JToken raw = fields[field];
                string value = raw == null || raw.Type == JTokenType.Null ? null : raw.ToString().Trim();
                if (string.IsNullOrEmpty(value)) { error = "information_container.fields." + field + " is missing or empty."; return null; }
                if (value.Contains(separator))
                { error = "information_container.fields." + field + " contains the separator '" + separator + "'."; return null; }
                string pattern = patterns?.Value<string>(field);
                if (!string.IsNullOrEmpty(pattern))
                {
                    bool ok;
                    try { ok = Regex.IsMatch(value, "^(?:" + pattern + ")$", RegexOptions.CultureInvariant, TimeSpan.FromSeconds(1)); }
                    catch (ArgumentException ex) { error = "information_container.field_patterns." + field + " is not a valid pattern: " + ex.Message; return null; }
                    if (!ok) { error = "information_container.fields." + field + " '" + value + "' does not match " + pattern + "."; return null; }
                }
                parts.Add(value);
            }
